...
def stationIO(
    ...
    with h5py.File(stationfilename, "r") as db:
        ...
        for _j, comp in enumerate(components):
            ...
            t0 = time()
            array = db[f"displacement/{comp}/array"][:, nglob2sub[sglob], :]
    ...
    with h5py.File(subsetfilename, "r+") as db:
        ...
        if fortran:
            db[f"displacement"][:, :, :, :, _i] = displacement[:, :, :, :].transpose(
                (3, 2, 1, 0)
            )
        else:
...
def stationIO_DB(
    ...
    with h5py.File(stationfilename, "r") as db, h5py.File(
        outstationfilename, "w"
    ) as dbout:
        # station = db['Station'][()].decode("utf-8")
        # network = db['Network'][()].decode("utf-8")
    ...
    * returns Stream() containing all three components
    ...
    with h5py.File(stationfile, "r") as db:
        ...
        for _i, comp in enumerate(["N", "E", "Z"]):
            ...
            displacementd[comp] = (
                db[f"displacement/{comp}/array"][:, iglob[sglob], :]
                .astype(np.float64)[:, rsglob, :]
                .reshape(3, NGLLX, NGLLY, NGLLZ, NT)
                * norm_disp
                / factor
            )
    ...
    for k in range(NGLLZ):
        for j in range(NGLLY):
            for i in range(NGLLX):
                ...
                hlagrange_x = (
                    hlagrange_xi * xix + hlagrange_eta * etax + hlagrange_gamma * gammax
                )
                hlagrange_y = (
                    hlagrange_xi * xiy + hlagrange_eta * etay + hlagrange_gamma * gammay
                )
                hlagrange_z = (
                    hlagrange_xi * xiz + hlagrange_eta * etaz + hlagrange_gamma * gammaz
                )
    ...
    _, stf_r = create_stf(
        0, 200.0, NT, dt, hdur_r, cutoff=None, gaussian=False, lpfilter="butter"
    )
    ...
    for _i, comp in enumerate(["N", "E", "Z"]):
        ...
        data = np.sum(
            np.array([1.0, 1.0, 1.0, 2.0, 2.0, 2.0])[:, None]
            * Mx[:, None]
            * np.squeeze(epsilon[_i, :, :]),
            axis=0,
        )

        # Convolution with Specfem Heaviside function
        data = np.real(fft.ifft(phshift * fft.fft(data, n=NP2) * dt * STF_R))[:NT]
        ...
        stats.coordinates = AttribDict(latitude=latitude, longitude=longitude)
...
def get_seismograms_sub(stationfile: str, cmt: CMTSOLUTION):
    ...
    * returns Stream() containing all three components
    ...
    with h5py.File(stationfile, "r") as db:
        ...
        for _i, comp in enumerate(["N", "E", "Z"]):
            ...
            displacementd[comp] = (
                db[f"displacement/{comp}/array"][:, iglob[sglob], :]
                .astype(np.float64)[:, rsglob, :]
                .reshape(3, NGLLX - 2, NGLLY - 2, NGLLZ - 2, NT)
                * norm_disp
                / factor
            )
    ...
    # Heaviside STF to reproduce SPECFEM stf
    _, stf_r = create_stf(
        0, 100.0, NT, dt, hdur_r, cutoff=None, gaussian=False, lpfilter="butter"
    )
    STF_R = fft.fft(stf_r, n=NP2)
    ...
    for _i, comp in enumerate(["N", "E", "Z"]):
        ...
        # Convolution with Specfem Heaviside function
        data = np.real(fft.ifft(phshift * fft.fft(data, n=NP2) * STF_R))[:NT] * dt
...
class GFManager(object):
    ...
    * Then, for inversion purposes use .get_seismograms(cmt) method to return
            ...
            if self.do_adjacency_search:
                if self.subset:
                    ...
                    if do_adjacency_search:
                        ...
                        logger.debug("Dont have buffer elements for doing the ")
    ...
    def get_stations(self):
        ...
        if self.subset:
            ...
            with contextlib.ExitStack() as stack:
                # Open Stack of files
                dbs = [stack.enter_context(h5py.File(fname, "r")) for fname in self.db]
        ...
        return [
            sta_tup
            for sta_tup in zip(networks, stations, latitudes, longitudes, burials)
        ]

    def get_elements(
        self, lat, lon, depth, dist_in_km=125.0, NGLL=5, threading: bool = True
    ):
        if self.subset:
            logging.warning(
        ...
        if isinstance(self.ispec_subset, np.int64):
            ...
            if len(self.ispec_subset) == 0:
                raise ValueError("Could not find any elements within the radius.")
            else:
        ...
        if self.subset:
            ...
            with contextlib.ExitStack() as stack:
                ...
                if NGLL == 3:
                    ...
                    raise ValueError(f"NGLL {NGLL} is not valid. Choose 3 or 5.")

                # Read ibool
                logger.info("Getting ibool subset ...")
                ibool = self.ibool[
                    iboolslice, iboolslice, iboolslice, self.ispec_subset
                ]
                ...
                if self.do_adjacency_search:
                    ...
                    for _j in range(len(self.ispec_subset)):
                        ...
                        # Get neighbors
                        num_neighbors = (
                            self.header["xadj"][ispec + 1] - self.header["xadj"][ispec]
                        )

                        # Loop over neighbors in full mesh
                        for i in range(num_neighbors):
                            # get neighbor from global adjacency
                            ispec_neighbor = self.header["adjacency"][
                                self.header["xadj"][ispec] + i
                            ]
                            ...
                            if ispec_neighbor in self.ispec_subset:
                                ...
                                idx = np.where(ispec_neighbor == self.ispec_subset)[0]
                ...
                if threading:
                    with parallel_backend("threading", n_jobs=10):
                        Parallel()(
                            delayed(read_stuff)(i) for i in zip(range(self.Ndb), dbs)
                        )
                else:
    ...
    def write_subset_directIO(
        ...
        # Read ibool
        logger.info("Getting ibool subset ...")
        ibool = self.ibool[iboolslice, iboolslice, iboolslice, self.ispec_subset]
        ...
        if self.do_adjacency_search:
            ...
            self.xadj = np.zeros(len(self.ispec_subset) + 1, dtype="i")
            MAX_NEIGHBORS = 50
            tmp_adjacency = np.zeros(MAX_NEIGHBORS * len(self.ispec_subset), dtype="i")
        ...
        with h5py.File(subsetfilename, "w") as db:
            ...
            if fortran:
                ...
                db.create_dataset("ibool", data=self.ibool.transpose((3, 2, 1, 0)))
            ...
            # Adjacency flag
            db.create_dataset("do_adjacency_search", data=self.do_adjacency_search)
            ...
            if self.do_adjacency_search:
                ...
                if fortran:
                    db.create_dataset("adjacency", data=self.adjacency + 1)
                else:
            ...
            if self.header["topography"]:
                ...
                db.create_dataset("RESOLUTION_TOPO_FILE", data=self.header["res_topo"])
            ...
            if self.header["ellipticity"]:
                ...
                db.create_dataset(
                    "ellipticity_spline", data=self.header["ellipticity_spline"]
                )
                db.create_dataset(
                    "ellipticity_spline2", data=self.header["ellipticity_spline2"]
                )
            ...
            dispshape = (self.Ndb, len(self.components), 3, len(self.nglob2sub), nsteps)
    ...
    def write_DB_directIO(
        self, DBdir, lat, lon, depth, dist_in_km=125.0, duration=None, NGLL=5
    ):
        if self.subset:
            logging.warning("Note that you already loaded a subset. Exiting")
            return
        ...
        # Header Variables
        if duration is not None:
            nsteps = int(np.ceil((self.header["tc"] + duration) / self.header["dt"]))
    ...
    def get_seismograms(self, cmt: CMTSOLUTION) -> Stream:
        ...
        (
            ...
            POINT_CAN_BE_BURIED=True,
            kdtree=self.kdtree,
            do_adjacent_search=self.do_adjacency_search,
            NGLL=self.NGLL,
        )
            ...
            for j in range(self.NGLL):
                for i in range(self.NGLL):
                    ...
                    hlagrange_x = (
                        hlagrange_xi * xix
                        + hlagrange_eta * etax
                        + hlagrange_gamma * gammax
                    )
                    hlagrange_y = (
                        hlagrange_xi * xiy
                        + hlagrange_eta * etay
                        + hlagrange_gamma * gammay
                    )
                    hlagrange_z = (
                        hlagrange_xi * xiz
                        + hlagrange_eta * etaz
                        + hlagrange_gamma * gammaz
                    )
        ...
        if (cmt.hdur / 1.628) ** 2 <= self.header["hdur"] ** 2:
            ...
            logger.warn(
                f"Try convolving your seismogram with a Gaussian with "
        ...
        shift = -200.0 + cmt.time_shift
        phshift = np.exp(
            -1.0j * shift * np.fft.fftfreq(NP2, self.header["dt"]) * 2 * np.pi
        )
        ...
        for _h in range(len(self.stations)):
            for _i, comp in enumerate(["N", "E", "Z"]):
                ...
                data = (
                    np.real(
                        fft.ifft(
                            fft.fft(seismograms[_h, _i, :], n=NP2) * STF_R * phshift
                        )[: self.header["nsteps"]]
                    )
                    * self.header["dt"]
                )
    ...
    def get_mt_frechet(self, cmt: CMTSOLUTION) -> tp.Dict[str, Stream]:
        ...
        for _j, mtc in enumerate(["Mrr", "Mtt", "Mpp", "Mrt", "Mrp", "Mtp"]):
            # Get lagrange values at specific GLL points
            M = mx[_j] * np.array([1.0, 1.0, 1.0, 2.0, 2.0, 2.0])
            ...
            for _h in range(len(self.stations)):
                for _i, comp in enumerate(["N", "E", "Z"]):
                    data = (
                        np.real(
                            fft.ifft(
                                STF_R * fft.fft(seismograms[_h, _i, :], n=NP2) * phshift
                            )
                        )[: self.header["nsteps"]]
                        * self.header["dt"]
                        / 1e23
    ...
    def get_mt_frechet_station(
        self, cmt: CMTSOLUTION, network: str, station: str
    ) -> tp.Dict[str, Stream]:
        idx = None
        ...
        if idx is None:
            raise ValueError(f"Station {network}.{station} not found in database.")
            ...
            for j in range(self.NGLL):
                for i in range(self.NGLL):
                    ...
                    for _c in range(3):
                        ...
                        epsilon[_c, 3, :] += 0.5 * (
                            displacement[idx, _c, 1, i, j, k, :] * hlagrange_x
                            + displacement[idx, _c, 0, i, j, k, :] * hlagrange_y
                        )
                        epsilon[_c, 4, :] += 0.5 * (
                            displacement[idx, _c, 2, i, j, k, :] * hlagrange_x
                            + displacement[idx, _c, 0, i, j, k, :] * hlagrange_z
                        )
                        epsilon[_c, 5, :] += 0.5 * (
                            displacement[idx, _c, 2, i, j, k, :] * hlagrange_y
        ...
        for _j, mtc in enumerate(["Mrr", "Mtt", "Mpp", "Mrt", "Mrp", "Mtp"]):
            ...
            for _i, comp in enumerate(["N", "E", "Z"]):
                data = (
                    np.real(
                        fft.ifft(STF_R * fft.fft(seismograms[_i, :], n=NP2) * phshift)
                    )[: self.header["nsteps"]]
                    * self.header["dt"]
                    / 1e23
                )
    ...
    def write_subset(self, outfile, duration=None, fortran=False):
        ...
        with h5py.File(outfile, "w") as db:
            ...
            if fortran:
                ...
                db.create_dataset(
                    "displacement",
                    data=self.displacement[:, :, :, :, :nsteps].transpose(
                        (4, 3, 2, 1, 0)
                    ),
                )
            else:
    ...
    def load(self):
        ...
        with h5py.File(self.headerfile, "r") as db:
            ...
            if fortran:
                self.displacement = self.displacement.transpose((4, 3, 2, 1, 0))
