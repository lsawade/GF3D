...
def locate_point(
    x_target, y_target, z_target, lat_target, lon_target, POINT_CAN_BE_BURIED, NSPEC,
    ibool, x, y, z, kdtree):
               ...
               do ispec = 1, nspec
                   ...
                   dist_squared = (x_target - xyz_midpoints(1, ispec))*(x_target - xyz_midpoints(1, ispec)) &
                       + (y_target - xyz_midpoints(2, ispec))*(y_target - xyz_midpoints(2, ispec)) &
                        + (z_target - xyz_midpoints(3, ispec)) * \
                        (z_target - xyz_midpoints(3, ispec))
...
#      endif
       ...
       call find_local_coordinates(x_target, y_target, z_target, xi, eta, gamma, x, y, z, &
        ...
        if (DO_ADJACENT_SEARCH) then
           ...
           if (np.abs(xi) > 1.0990 . or . np.abs(eta) > 1.0990 . or . np.abs(gamma) > 1.0990) then
...
#
    ...
    #     if (np.abs(xi) < 1.0990 . and. np.abs(eta) < 1.0990 .and. np.abs(gamma) < 1.0990) exit
