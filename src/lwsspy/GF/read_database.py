...
with adios2.open(reciprocal_file, "r", comm) as rh:
    ...
    with adios2.open(forward_file, "r", comm) as fh:
        ...
        if (rank == 0):
            ...
            repsilon_xx = Variable(fh, 'epsilon_xx', block_id=0, nsteps=nsteps)
            repsilon_yy = Variable(fh, 'epsilon_yy', block_id=0, nsteps=nsteps)
            repsilon_zz = Variable(fh, 'epsilon_zz', block_id=0, nsteps=nsteps)
            repsilon_xy = Variable(fh, 'epsilon_xy', block_id=0, nsteps=nsteps)
            ...
            plt.figure()
            plt.plot(sepsilon.T)
            plt.savefig('eps_interp.png', dpi=300)
            # sys.exit()
            ...
            ibool_GF.array = ibool_GF.array.reshape((5, 5, 5, -1), order='F')
            ...
            for i in range(3):
                ...
                plt.subplot(3, 1, i+1)
                plt.title(
                    f'{st[i].id} - event (lat,lon,dep)= (35.1500,26.8300,20.0), Crete,Greece', fontsize='small')
                plt.plot(s[idx[i], :], 'k', lw=0.75, label='GF')
                plt.plot(st[i].data, 'r--', lw=0.75, label='SF3D_GLOBE')
