...
def simulation(node: Node):
    ...
    for comp in ['N', 'E', 'Z']:
        ...
        node.add_mpi('./bin/xspecfem3D', nprocs=24, gpus_per_proc=4, mps=6,
                     cwd=compsimdir,
                     name=f'sim-{node.network}.{node.station}.{comp}-{compsimdir}')
